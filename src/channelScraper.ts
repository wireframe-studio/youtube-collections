...
export async function scrapeChannels(): Promise<Channel[]> {
  ...
  for (const element of channelElements) {
    try {
      const channelLink = element.querySelector<HTMLAnchorElement>('#main-link');
      const channelName = element.querySelector<HTMLElement>('#text')?.textContent?.trim();
...
export function showScrapeProgress(): HTMLDivElement {
  ...
  container.className = 'fixed bottom-4 right-4 bg-gray-900 text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 z-[10000]';
  container.innerHTML = `
