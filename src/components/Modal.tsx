...
export function Modal({ onClose }: ModalProps) {
	...
	return (
		<view
			className="fixed inset-0 bg-black/25 backdrop-blur-xs flex items-center justify-center z-10000 p-4"
			onClick={onClose}>
			<view
				className="bg-[#282828] rounded-3xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-2xl border border-white/10 mx-4"
				onClick={(e) => e.stopPropagation()}>
				{/* Header */}
				<view className="flex items-center justify-between px-8 py-6 border-b border-white/10">
